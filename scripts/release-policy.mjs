...
export function validateManifest(body, { architecture, dmg, tag, now = new Date() }) {
  ...
  requireValue(body.platform === "macos", "unexpected manifest platform");
  ...
  requireValue(artifact.platform === "macos", "unexpected artifact platform");
  requireValue(artifact.architecture === architecture, "artifact architecture mismatch");
  requireValue(artifact.format === "dmg", "unexpected artifact format");
  requireValue(artifact.url === dmg.url, "manifest URL does not match release asset");
  requireValue(artifact.sha256 === dmg.sha256, "manifest digest does not match release asset");
  requireValue(artifact.size === dmg.size, "manifest size does not match release asset");
  requireValue(
    artifact.file_name === basename(new URL(dmg.url).pathname),
    "manifest filename does not match release asset",
  );
  requireValue(/^[a-f0-9]{64}$/.test(artifact.sha256), "invalid artifact digest");
  requireValue(Number.isSafeInteger(artifact.size) && artifact.size > 0, "invalid artifact size");
